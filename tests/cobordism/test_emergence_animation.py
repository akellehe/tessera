...
class DriveTest(unittest.TestCase):
    ...
    def test_the_default_seed_is_a_euclidean_spatial_complex(self):
        """A host is a level, and in the whitepaper (v16, section 3) time is
        the fibering direction between levels, realized only by the fiber
        edges of the interaction cobordism, not a simplex dimension of any
        level. The default seed is therefore purely spatial, of Euclidean
        signature: every squared length is +1. Nothing constrains the
