...
is the T1-T3 setup: two ``SimplicialQubit.flat_torus`` inputs,
...
* the config refuses a modulus off the upper half plane, a grid below 3, a
  non-positive weight, and the CLI parses the
...
def _run():
    ...
    if "result" not in _CACHE:
        seen = []
        config = qa.build_config(steps=STEPS, stage1_iters=1, stage2_iters=2,
                                 tolerance=1e-30,
...
def test_one_frame_per_unit_published_live():
    ...
    assert run["result"].terminator == qa.Terminator.STEPS
...
def test_every_channel_is_a_measurement_or_a_named_absence():
    for frame in _run()["result"].frames:
        for name in qa.QubitFrame.QUBIT_CHANNELS + ("betti", "objective"):
            value = getattr(frame, name)
            if isinstance(value, qa.Absent):
        ...
        for row in frame.blocks:
            assert row["label"] in qa.DECLARED_TORUS_LABELS
        ...
        for name in qa.QubitFrame.CERTIFICATE_CHANNELS:
            value = getattr(frame, name)
            assert isinstance(value, qa.Absent) and "qubit input mode" in value.reason
        assert isinstance(frame.dual, qa.Absent) and "3-dimensional" in frame.dual.reason
...
def test_frame_zero_is_the_collar_seed():
    ...
    assert inputs.weight == qa.DECLARED_INPUT_WEIGHT and inputs.regge
    assert inputs.seed == {"cells": 54, "vertices": 18, "edges": 90}
    for index, tau_in in enumerate((TAU_A, TAU_B)):
        row = frame.blocks[index]
        assert row["tau_in"] == tau_in and row["weight"] == qa.DECLARED_INPUT_WEIGHT
    ...
    expected = two_body["residual"] + qa.DECLARED_INPUT_WEIGHT * sum(frame.blocks[i]["residual"] for i in range(2))
    ...
    print("\n[T4] seed: blocks %s, output leaks %s (weight %g), coefficients %s, leaks %s, two-body %.6f, T %s" % (
        ...
        ["%.3e" % frame.blocks[i]["output_leak"] for i in range(2)], qa.DECLARED_INPUT_WEIGHT,
...
def test_the_flip_flop_algebra():
    ...
    chi = qa.two_qubit_flip_flop(psi, phi)
    ...
    small = qa.flip_flop_evolution(psi, phi, 2.0, 5e-5)
    ...
    large = qa.flip_flop_evolution(psi, phi, 1.0, 0.7)
...
def test_to_json_round_trips_with_complex_numbers_as_pairs():
    ...
    for frame in run["result"].frames:
        ...
        for name in qa.QubitFrame.QUBIT_CHANNELS:
    ...
    assert len(inputs["algebra"]["chi"]) == 2 and inputs["algebra"]["Jt"] == qa.DECLARED_COUPLING * qa.DECLARED_TIME
...
def test_render_png_and_gif_under_agg():
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        qa.render(frames, png)
        qa.render(frames[:2], gif)
        qa.render(frames[:1], one)
    ...
    try:
        qa.draw_frame(figure, frames, len(frames) - 1)
        ...
        plt.close(figure)
    assert [name for name, _ in qa.panels_for(_run()["config"])][:6] == list(qa._QUBIT_PANEL_ORDER[:6])
    rows, columns = qa.DECLARED_PANEL_GRID
    assert len(qa._PANELS) <= rows * columns
    assert {name for name, _ in qa._QUBIT_PANELS} <= {name for name, _ in qa._PANELS}
...
def test_the_cli_parses_the_qubit_flags():
    ...
    args = parser.parse_args(["run", "--tau-a", "0.3+1.1j", "--tau-b=-0.2+0.8j",
                              ...
                              "--no-regge"])
    assert args.tau_a == TAU_A and args.tau_b == TAU_B
    ...
    assert defaults.regge is qa.DECLARED_REGGE
    assert defaults.tau_a == qa.DECLARED_TAU_A and defaults.grid == qa.DECLARED_GRID
    with pytest.raises(argparse.ArgumentTypeError):
        qa._complex_argument("abc")


def test_main_writes_the_run_document():
    with tempfile.TemporaryDirectory() as directory:
        path = os.path.join(directory, "run.json")
        qa.main(["run", "--tau-a", "0.3+1.1j", "--tau-b=-0.2+0.8j",
    ...
    assert document["terminator"] in qa.Terminator.ALL
