...
def test_the_mean_field_of_a_span_is_the_wick_contraction_of_the_library(span):
    ...
    integrals = acceleration.SpanIntegrals(mesh.kernel, mesh.cell, modes)
    ...
    assert np.abs(2.0 * coulomb.pair_loads(mesh.cell, filled[:, 0], filled)[:, 0] - integrals.density_load(frame)).max() < 1e-13
