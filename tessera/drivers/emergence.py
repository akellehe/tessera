...
# All rights reserved.
"""Animate unforced emergence and the certificates the paper names.
...
#: relaxation -- the engine's deterministic unit.
DECLARED_STEPS = 6
...
#:
#: Spacelike, because a host is a level. In the whitepaper (v16, section 3)
#: time is the fibering direction: the passage of a certified cluster at level
#: l to a response vertex at level l + 1, along which its fiber is transported,
#: realized only by the fiber edges of the interaction cobordism. Time is
#: therefore not a simplex dimension of any level, so a level carries no time
#: component and is purely spatial, of Euclidean signature 3 + 0. Its Gram
#: matrices are real positive definite, so it lies inside the Kontsevich-Segal
#: allowable domain at margin pi and has no timelike part to rotate. For
#: comparison at size 4: the foliated seed lies on the boundary of the domain
#: (margin 0) and inside it once rotated (0.126 at `DECLARED_EPSILON`); the
#: `random` seed lies outside (-4.91) and declares no timelike part;
#: `timelike` and `lightlike` lie outside at every rotation
...
def build_cobordism_host(n_refine=DECLARED_SIZE, seed=DECLARED_HOST_SEED,
    ...
    manifold. The paper prescribes no host topology; it specifies
...
def main(argv=None):
    ...
    if legacy_inputs == "qubit":
        ...
        # Imported lazily: qubit imports this module at import time, so a
        # module-level import here would close the cycle.
        from tessera.drivers import qubit
        return qubit.main(arguments)
