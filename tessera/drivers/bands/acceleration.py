...
class SpanIntegrals:
    ...
    def __init__(self, kernel, triple, modes, block=64):
        self.modes = np.asarray(modes, dtype=float)
        size, count = self.modes.shape
        self.count, self.triple = count, triple
        ...
        for p in range(count):
            self.loads[:, self.index[p, p:]] = triple.loads(self.modes[:, p], self.modes[:, p:])
    ...
    def exchange(self, frame, zero_momentum):
        ...
        for j in range(frame.shape[1]):
            W -= self.triple.loads(filled[:, j], pair[j])
        return W
