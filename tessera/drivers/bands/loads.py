...
class LocalLoads:
    ...
    def derivative(self, axis, momentum=None):
        """The derivative of `assemble` with respect to the Cartesian momentum
        component `axis`, at the zone centre or at the Cartesian `momentum`."""
        values = self.values if momentum is None \
            else self.values * np.exp(-1j * (self.displacement @ np.asarray(momentum, dtype=float)))[:, None]
        return self._scatter(-1j * self.displacement[:, axis, None] * values)
