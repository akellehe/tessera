...
\large Derivation and experiment specification}
\author{}
\date{}
...
\begin{abstract}
This document specifies the harmonic-operator experiment: its analytical
derivation, geometric operator algebra, theta tensor register, and
root-system extension. The objective is an exact representation of
finite-dimensional algebra by quantized geometry, not synthesis of every
operator as one engine-generated spacetime. Harmonic forms parameterize a
classical phase space; quantization supplies the quantum Hilbert space.
Geometric translations and their complex linear combinations form its full
matrix algebra, preserving tensor products, adjoints, states and operator
vectors. Root-lattice geometry represents characters and fusion with
explicitly declared quantization data. Single-cobordism synthesis and
physical dynamics are separate questions. Theorems, measurements and open
constructions are distinguished throughout.
\end{abstract}

\begingroup\small
\section*{Scope and notation}
\addcontentsline{toc}{section}{Scope and notation}
The source is \texttt{docs/design/cobordism\_functor\_reading.tex} in the
Tessera repository; the adjacent PDF is generated from it, and the copies
in \texttt{\string~/tessera-notes/cobordism-functor/} are reading copies.
Open work is tracked on GitHub, not here. The companion map, problems
(P1--P83) and solutions are study aids; where they conflict with this
...
The primer uses $k$ for cochain degree in its Hodge discussion and for
the positive integer quantization level in its theta/affine discussion;
those are independent quantities. In $\mathfrak{su}(N)$, $N$ is the defining
...
where that bridge applies; $M_p$ denotes a mass matrix. $\Omega_\partial$
is the boundary intersection form; $\Omega$ is a complex period matrix.
Transpose and Hermitian adjoint are never interchangeable without the
stated real or duality hypotheses.
\endgroup
\clearpage
\tableofcontents
\clearpage
...
three levels. The goal is an equivalent geometric representation
...
whose native readout realizes a specified operator. The experiment
...
$\Omega_\partial = J_A \oplus \varepsilon J_B$ where $\varepsilon = \pm 1$ records that
...
for. Problem~P6 shows why it cannot be read off $\det M$ once
...
$\Hom(A,B) \cong B\otimes A^*$: a map $T$ corresponds to the vector
\[
  |T\rangle\!\rangle \;=\; \sum_i T e_i \otimes e_i^* ,
\]
...
input factor, in the same output-major order as
Section~\ref{sec:quantum-contract}.
...
valid graph and need not be invertible.
...
and dual transports coincide; otherwise this requires proof.
\end{proof}
...
represented by $\sum_i da_i\wedge db_i$.
\end{definition}
...
\emph{is} doubly periodic (Problem~P21), and each $\theta_j$ has
exactly $k$ zeros in a fundamental domain (Problem~P22).
...
validate; matrices are independent oracles.
...
The antiunitary lift is part of the data.
...
$\Omega_{\rm in}$. Measure the holonomy relation separately.
Different-modulus identity collars are negative controls:
...
\begin{remark}[Measured seams]\label{rem:seam-measured}
Section~\ref{sec:evidence-merged} records three constructions. A
$1$-handle between the far tori of two collars (a tube over one triangle
of each) makes the far boundary one genus-two surface; its monodromy stays
block-diagonal, while the surface's own period matrix acquires an
off-diagonal entry $\Omega_{12}$ set by the neck's waist and length, and
sampled coherent states have nonzero entropy in the declared two-factor
register. Layered flip passes on a collar's far torus read the Dehn twists
$T_A$, $T_B$ and the quarter turn $S$, and stacked passes compose in order.
The collar of the $\Z/10$-symmetric genus-two surface relabelled by its
order-five rotation reads an integer symplectic monodromy of order five
that acts on the level-two register as an entangling two-qubit Clifford
gate, by the product-input witness above. A Dehn twist along a curve
through the tube's neck --- the controlled-$Z$ shear on a torus-pair
...
automatically a non-Clifford unitary.
...
set of unitary gates need not be dense.
...
\begin{example}[$\mathfrak{su}(2)$; Problem P48]\label{ex:sl2}
...
\begin{example}[$\mathfrak{su}(3)$; Problem P49]\label{ex:sl3}
...
(Section~\ref{sec:root-program}); a generic boundary triangulation need
not have this symmetry, nor its harmonic mass Gram equal $A$.
...
\begin{theorem}[Weyl character formula; Problem P52]\label{thm:weylchar}
For $\lambda\in P^+$,
\[
...
\begin{proposition}[Brauer--Klimyk; weights add, the Weyl group folds; Problem P53]
\label{prop:klimyk}
...
$\langle\mu,\theta^\vee\rangle = K$ (Problem P55). At level $K$ the
fundamental alcove
...
\begin{theorem}[Weyl--Kac; the lattice sum is the translation orbit; Problem P56]\label{thm:weylkac}
...
\]
so, with $A^{(K)}_\mu = \sum_{w\in W}\varepsilon(w)\,\Theta^{(K)}_{w\mu}$,
the numerator at $K = k+h^\vee$ is $A^{(K)}_{\lambda+\rho}$ up to a power of
$q$, the denominator is $A^{(h^\vee)}_{\rho}$ (the affine denominator
identity), and the normalized level-$k$ character with highest weight
$\lambda\in P_k^+$ is
\[
...
\begin{theorem}[Sugawara; Problem P57]\label{thm:sugawara}
...
\begin{proposition}[Rank-two Weyl groups are the symmetries a torus can have; Problem P59]
...
by simplicial automorphisms --- the diagonal breaks the fourfold symmetry
--- which is abelian, so composition order is invisible among
automorphisms; the layered flip passes of Section~\ref{sec:evidence-merged}
are not automorphisms and read the infinite-order twists $T_A$, $T_B$ and
the quarter turn $S$ on the same grid. The triangular (hexagonal) lattice
...
By Theorem~\ref{thm:weylkac} the character is the ratio
$\chi_\lambda = A^{(k+h^\vee)}_{\lambda+\rho}/A^{(h^\vee)}_{\rho}$: the sum
over $Q^\vee$ is the translation part of $\hat W$, the sum over $W$ its
finite part (Problems P62, P63). For $\mathfrak{su}(2)$ ($r=1$, $W=\{\pm1\}$, $h^\vee = 2$) this is fully
...
matrix of Theorem~\ref{thm:sugawara} (Problems P64, P57).
...
validates the representation objective.
...
ingredient is the affine reflection $s_0$ (Problem P65).
...
integral nonnegative structure constants.

\section{Simply-laced level one and the abelian lattice register}\label{sec:frenkelkac}

\begin{theorem}[Frenkel--Kac--Segal; Problem P58]\label{thm:fk}
...
Punctures and braids are the proposed route. Removing a bulk top cell does
not by itself construct a labeled surface puncture or conformal-block
Hilbert space, and the Weyl isotypic sectors of an operator are not alcove
labels. Section~\ref{sec:root-program} states what a realization must supply.
...
This section states the native harmonic-recovery contract. Its three
verdicts are identifiable operator, positive norm preservation, and
requested native gate realization; they are separate from the
representation verdict of Section~\ref{sec:validation}.
...
describe the same period-normalized pairing; primal-frame and biorthogonal
observation coordinates are distinct.
...
algebraic control, not scalar-geometric synthesis. Actual composition must
identify interface cells/frames and compare the assembled glued relation.
Extra dimension does not by itself give matrix realizability: a connected
positive scalar degree-zero connection graph has at most one zero mode,
since $f_j=e^{i\theta_{ij}}f_i$ fixes all values from one vertex and
inconsistent holonomy forces zero.
...
frames, or the $2\times2$ selected-pair $\chi$, is not that operator.
Dimension matching alone does not define subsystems.

\section{Representation acceptance contract}\label{sec:validation}

The deliverable is a representation theorem with computational checks of
its hypotheses and operations, not a dynamics or synthesis claim. Theorems
hold exactly under their assumptions; numerical residuals measure
implementation accuracy. Each objective identifies which operations are
...
\item Geometric operator algebra: represent $e_{p,q}$ by scalar
translations; implement reduced cycle/translation symbols, scalar crossing
phases, involution, trace and twisted convolution. Verify the crossing law,
involution, the $d^2$ independent symbols, the trace pairing, forward and
inverse maps for arbitrary operators, adjoints and multiplication against
independent matrices, including non-Clifford and nonunitary operators.
Keep the pinned-boundary defect as a distinct invariant control, not an
optimization target; add no relaxation to the fixed-complex verifier.
\item Holonomy-defined states: keep the neutral harmonic pencil, construct
ordinary closed phase configurations and read Wilson loops independently;
verify gauge and large-gauge invariance and neutral-pencil independence.
Fix $z=b-\Omega a$, the coordinate bridge, the conjugate coherent action
and the antiunitary factorization. Demonstrate qubit coverage with several
target rays including zeros and poles, and its non-unique inverse. At level
three use a named characteristic-preserving subgroup or track
characteristics explicitly; do not claim coherent coverage of every qutrit
ray.
\item Tensor and Choi algebra: verify tensor-factor ordering, matrix units,
symbol tensor products and dual-factor contraction against composition,
including rectangular maps, product, SWAP and controlled-$Z$ controls, an
entangling witness and the state/channel distinction. Keep the wider-neck
geometry study (Section~\ref{sec:neck-entropy}) a separate representation
and entropy experiment with paired samples, period accuracy and
refinement, and logarithmic entropy asymptotics; it is neither a
requirement for the algebra theorem nor a claim about Ryu--Takayanagi.
\item Root-lattice representation: recover $A_2$, its roots, reflections
and the weight lattice from scalar lattice lengths; use the nonprincipal
bundle and the $P/KQ^\vee$ characteristic set of
Section~\ref{sec:root-coordinates}. Fit normalized Weyl--Kac characters
against Kac--Peterson on held-out points; compare fusion by geometric
alcove evaluation, Verlinde and independent Kac--Walton folding, with $A_1$
and the even-lattice Frenkel--Kac controls. Host-derived $\tau$ is an input
source; the native fiber need not be synthesized.
...
\section{Native implementation and acceptance contract}\label{sec:implementation}

These requirements govern the native-engine path where it is exercised;
they do not replace V1--V8. The verifier's check names (R1--R5, H1--H3,
J1--J3, C1, D1, U1, F1--F4, G1--G5, E1--E6, T1--T9) are a separate
namespace from the contract items below. Incorrect scientific claims are
corrected with regressions, not preserved as acceptance criteria.

\subsection{Construction NC1--NC7}
\begin{enumerate}[label=NC\arabic*.,leftmargin=*]
...
\subsection{Scientific readout NR1--NR7}
\begin{enumerate}[label=NR\arabic*.,leftmargin=*]
...
\item Record the actual objective: transfer/$\chi$ fitting
...
\texttt{observation\_convention: dual\_frame\_whitney}. Records without
this field used primal frames; normalizations need not agree even
at zero phase. Identity coordinate norms are explicit external encodings.
\item Preserve $\chi$, not as a full XY/two-qubit gate and
...
\subsection{Compatibility NK1--NK5}
\begin{enumerate}[label=NK\arabic*.,leftmargin=*]
...
the $\chi$ measurements while displaying whole-operator evidence.
...
\subsection{Acceptance NA1--NA9}
\begin{enumerate}[label=NA\arabic*.,leftmargin=*]
...
\item Preserve the tube, flip and decagon reads (Section~\ref{sec:evidence-merged}):
...
Native fiber synthesis is optional. The internal coroot lattice is
...
The acceptance claim (V5) is that the lattice/alcove construction
reproduces the character and fusion algebra. Native synthesis would add a
sector map, certified whole-kernel readouts and actual sewing; braiding
would add labeled punctures and compatible associativity and braiding maps.
...
The star-tube experiment uses wider disk attachment, waist/length sweeps
and larger grids, with $|\Omega_{12}|\sim0.3$--$0.5$ as an exploration
target. Record full $\Omega$, markings,
...
Total-system entropy against engine units and curvature/charge heatmaps
are not operator-vector entropy. Before reporting them, declare a positive
normalized joint $\rho$, subsystem factors and an evolution or
coarse-graining rule. Record
...
\texttt{qubit\_animation.py verify} reads a seeded collar at tolerance
$10^{-12}$: rank and integrality of $M$ (R1--R4);
isotropy for the declared form on the orthonormalized restriction subspace
with the flipped control nonvanishing (R5, Remark~\ref{rem:sign} and P6);
...
twist-group composition (C1, P14); the four-torus direct sum (D1,
...
(T1--T8, Sections~\ref{sec:weil}--\ref{sec:seam}). These are fixture
results, not proofs by numerical precision. Residuals lie at
$10^{-16}$--$10^{-14}$; metric-dependent defects can be order one or small
...
measured & Bilinear transport defect $0.46$--$0.58$ generically, about
...
open & Geometric operator algebra, holonomy/coherent encoding, tensor/Choi
composition and neck entropy, root-lattice characters and fusion
(V2--V5).\\
...
\subsection{Numerical evidence on the collar fixtures}
For grid $3$, seed $7$ and tolerance $10^{-12}$, \texttt{verify} and
\texttt{theta} report:
\begin{itemize}
...
off-block norm at most $5\,10^{-15}$. The automorphism twists of the
square triangulation commute; composition order is tested by the flip
passes (F4, T9).
...
\end{itemize}
These fixed-complex reads establish no engine dynamics, interior-seam
composition or non-abelian register; a triangulation-preserving search
move can leave the marked relation unchanged.

\subsection{Measurements on the tube, flip and decagon hosts}\label{sec:evidence-merged}
On the fixed complexes named, tolerance $10^{-12}$, grid $3$ and seed $7$
unless stated; records under \path{~/cobordism-runs/functor-verify/} and
\path{~/cobordism-runs/theta-register/}.
\begin{itemize}
\item \textbf{Tube-joined collars} (\texttt{theta --tori 4 --join tube}).
...
The entropy range is $5\,10^{-14}$ to $2.4\,10^{-3}$ bits mean; its
proportionality to $|\Omega_{12}|^2$ over this range is a finite-range
observation, not the asymptotic law.
...
entropy--area law; the averaged values are not paired-sample convergence
evidence. The asymptotics and controls are in
Section~\ref{sec:neck-entropy}.
\item \textbf{Dehn twists by layered flips}
...
the products of their passes, $T_AT_B=\begin{psmallmatrix}1&1\\1&2\end{psmallmatrix}$
and $T_BT_A=\begin{psmallmatrix}2&1\\1&1\end{psmallmatrix}$ (F4), with
Weil matrices at projective distance $1.0$ (T9); with the swap,
...
\item \textbf{The entangling collar} (\texttt{theta --surface decagon}).
...
\paragraph{Modular conventions of the theta records.}
The theta records fit $\rho$ from the period matrix directly (the
direct-monodromy input convention). They certify the recorded fits and
witnesses, not covariance with the simultaneously reported boundary
modulus; covariance tests apply Propositions~\ref{prop:coordinate-bridge}
and~\ref{prop:coherent-covariance}, distinguishing raw period matrices,
standard Siegel matrices, and value/state actions. A discrete period read
is not an exact Riemann-surface theorem by symmetry and positivity alone.
