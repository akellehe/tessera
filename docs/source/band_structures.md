...
| `--momenta` | the momentum set on which the covariance is sampled, a uniform grid of that many crystal momenta per axis of the cell through its zone centre (`momentum_set`); Hartree-Fock is solved at one momentum of every orbit of time reversal and of the axis permutations the crystal has, the screened interaction is built at every momentum transfer of the set with the entry at zero transfer in closed form, and the offsets of `--zero-momentum-order` surround every transfer. A cell on a set is the supercell at its zone centre, and the test suite holds every step to that identity | at least 1, default 1 (the zone centre); on a set `--self-energy-order` is implemented at 1 only |
